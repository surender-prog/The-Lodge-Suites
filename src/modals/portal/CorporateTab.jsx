...
export const CorporateTab = () => {
  ...
  return (
    <div>
      ...
      {/* Self-registered accounts awaiting activation — surfaced here because
          a fresh registration (0 spend) never makes the top-accounts table. */}
      {agreements.some((a) => a.status === "pending-approval") && (
          ...
          <div className="mt-3 flex flex-col gap-2">
            {agreements.filter((a) => a.status === "pending-approval").map((a) => (
              <div key={a.id} className="flex items-center gap-3 flex-wrap" style={{ fontFamily: "'Manrope', sans-serif", fontSize: "0.86rem" }}>
                ...
                <button
                  onClick={() => {
                    upsertAgreement({ id: a.id, status: "active", signedOn: a.signedOn || new Date().toISOString().slice(0, 10), startsOn: a.startsOn || new Date().toISOString().slice(0, 10) });
                    pushToast({ message: `Account activated · ${a.account} — they can now sign in` });
                  }}
