...
import { useData, ADMIN_ROLES, INTRO_TEMPLATE_ID, INTRO_TEMPLATE_SEED } from "../../data/store.jsx";
...
// Absolute URL to the property logo, resolved from the live deployment origin
// so the recipient's mail client loads it from the same host that serves the
// app. Computed once at module load (Vite SPA — window always exists at runtime).
const LOGO_URL = (typeof window !== "undefined" && window.location)
  ? `${window.location.origin}/images/logo.png` : "";

// Humanise a staff access-role slug ("fom" → "Front Office Manager") for the
// email signature. Access roles are NOT job titles, so this is only a
// last-resort fallback when the sender has no explicit title on file.
function roleLabel(roleId) {
  const r = (ADMIN_ROLES || []).find((x) => x.id === roleId);
  return r ? r.name : "";
}
...
export function IntroEmailModal({ activity, onClose }) {
  ...
  const {
    ...
    emailTemplates, upsertEmailTemplate,
  } = useData();

  const owner = useMemo(() => {
    // The signature must show the sender's JOB TITLE (e.g. "Front Office
    // Manager"), never the access-control role slug ("owner"/"fom"). Prefer
    // the directory record's title (what the admin Staff list shows), then the
    // session's own title, then a humanised role label as a last resort.
    const rec = (adminUsers || []).find((u) => u.id === (staffSession?.id || activity?.ownerId));
    if (staffSession?.id) {
      return {
        name:  staffSession.name || rec?.name || "",
        title: rec?.title || staffSession.title || roleLabel(staffSession.role),
        email: staffSession.email || rec?.email || "",
      };
    }
    return rec ? { name: rec.name, title: rec.title || roleLabel(rec.role), email: rec.email } : {};
  }, [staffSession, adminUsers, activity?.ownerId]);

  // The editable subject/body live in the "Partner introduction" template in
  // Settings → Email Templates, so operators manage the wording there. This
  // composer reads it as the text source; the branded HTML (offer callout +
  // logo/address footer) is always generated by buildIntroEmail on send.
  const introTpl = useMemo(
    () => (emailTemplates || []).find((t) => t.id === INTRO_TEMPLATE_ID) || null,
    [emailTemplates],
  );
  ...
  // Save / reset the shared "Partner introduction" template that lives in
  // Settings → Email Templates. Reverse-substitute resolved values back to
  ...
  const resetTemplate = () => {
    if (!window.confirm("Reset the Partner introduction template back to the built-in wording? Your custom edits will be discarded.")) return;
    upsertEmailTemplate({ id: INTRO_TEMPLATE_ID, subject: INTRO_TEMPLATE_SEED.subject, body: INTRO_TEMPLATE_SEED.body });
    pushToast({ message: "Template reset to the built-in wording." });
  };
